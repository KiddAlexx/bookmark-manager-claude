...
This project uses **Zustand + a service layer** for all data access. Do not introduce
TanStack Query or any other data-fetching library.

- All bookmark reads and writes go through `/src/lib/services/bookmarks.ts`
- The Zustand store (`/src/store/bookmarkStore.ts`) calls the service layer
- Components read from the Zustand store via selectors — they do not fetch directly
- This pattern holds in both Phase 1 (localStorage) and Phase 2 (Neon/Drizzle);
  only the service layer implementation changes between phases
